...
# Usage example
config_path = "configs/inference_dresscode_config.yaml"  # Configuration file path
...
def main():
    ...
    test_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=partial(collate_fn, order="test"),
    )
    ...
    for category in categories:
        os.makedirs(os.path.join(config.output_dir, config.inference_grid.format(config.order), category), exist_ok=True)
        os.makedirs(os.path.join(config.output_dir, config.inference_one.format(config.order), category), exist_ok=True)
    # Inference mode
    unet.eval()
    with torch.no_grad():
        with accelerator.autocast():
            ...
            for idx, batch in enumerate(test_dataloader):
                ...
                for i in range(len(image)):
                    ...
                    face = batch["stitch_label"][0].to(model_img.device)
                    face = transforms.functional.resize(face, 
                            size=(512,384), 
                    ...
                    final_img = compose_img_dresscode(
                        gt_img = model_img, 
                        fake_img = transforms.functional.to_tensor(image[i]).to(model_img.device), 
                        im_head = face
                    )
                    final_img1 = transforms.functional.to_pil_image(final_img)
                    final_img1.save(os.path.join(config.output_dir,config.inference_one.format(config.order),batch["category"][i],batch["im_name"][i]))
                    all_img = []
                    ...
                    to_pil_image(grid_image).save(os.path.join(config.output_dir,config.inference_grid.format(config.order), batch["category"][i], batch["im_name"][i]))
