...
# Usage example
config_path = "configs/train_vitonhd_config.yaml"  # Configuration file path
...
def compose_img(gt_img, fake_img, im_parse):
    """
    Merge real head image and generated body image based on parsing map.

    Args:
    gt_img: Ground truth image.
    fake_img: Generated fake image.
    im_parse: Parsing map used to identify different body parts.

    Returns:
    Composed image containing real head and generated other body parts.
    """
    # Create a logical mask to identify head regions (including hat, hair, sunglasses and mouth)
    ...
    # Extract head region from the real image
    true_head = gt_img * seg_head
    true_parts = true_head

    # Convert generated fake image to tensor, scale to [0,1] range, then extract non-head regions
    generated_body = (transforms.functional.pil_to_tensor(fake_img).cuda() / 255) * (~(seg_head))

    # Compose image: real head + generated other body parts
...
def main():
    ...
    accelerator.init_trackers(config.wandb_project)
            ...
            for _, batch in enumerate(train_dataloader):
                with accelerator.autocast():
                    with torch.no_grad():
                        ...
                        # Our experience shows that image_latent cat 0 works better than cat texture
                    ...
                    if (global_step + 1) == config.max_train_steps:
                        if accelerator.is_main_process:
                            save_path = os.path.join(config.output_dir, f"vitonhd_checkpoint-{global_step}-step")
